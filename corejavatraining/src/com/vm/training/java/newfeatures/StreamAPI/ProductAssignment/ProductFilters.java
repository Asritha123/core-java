...
public class ProductFilters {
	...
	public void productCatogery(String catogery)
	{
		if(catogery.equalsIgnoreCase("all"))
		{
			System.out.println();
		...
		products.stream().filter(p->p.getCategory().equalsIgnoreCase(catogery)).forEach(p->System.out.println(p.getPname()+"  "+p.getPrice()+"/-"));
		System.out.println("------------------------------------");
	}
	}
	...
	public void productCustomQuery(int quantity,String catogery1)
	{
		List<String> query=	products.stream().filter(p->p.getQuantity()>quantity&&p.getCategory().equalsIgnoreCase(catogery1)).map(p->p.getPname()+" "+p.getPrice()).collect(Collectors.toList());
		System.out.println("Product by Custom Query :");
		System.out.println("Quantity > "+quantity+" Catogery: "+catogery1);
